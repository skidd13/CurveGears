...
For generated catalogues, `-s path` adds a source link to each module heading.
The `-i` option infers a small module entry from the first ordinary `//`
comment when a file contains no Doxydown block. This keeps executable examples
and lightweight test fixtures in the same source-driven documentation flow.
