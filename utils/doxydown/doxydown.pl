...
sub parse_function {
    ...
    foreach ( @data ) {
        if ( /^\s*\@param\s*(?:\{([^}]+)\})?\s*(\S+)\s*(.+)?\s*$/ ) {
            ...
            $f->{'brief'} = $1;
        } elsif ( /^\s*\@image\s+(\S+)(?:\s+(.+))?\s*$/ ) {
            push @{ $f->{'images'} }, { path => $1, alt => $2 };
        }
...
sub parse_module {
    ...
    $f->{'source'} = $source_link if $source_link;
...
usage: $0 [-hg] [-l language] < input_source > markdown.md
 ...
 -s        : source path/link to print for each generated module
 -i        : infer a module from the first line comment when no block exists
EOF
...
getopts( 'he:gl:s:i', \%options );

HELP_MESSAGE() if $options{h};

$example_language = $options{e} if $options{e};
$source_link = $options{s} if $options{s};
$infer_module = 1 if $options{i};
...
while ( <> ) {
    if ( $state == STATE_READ_SKIP ) {
        if ( $infer_module && !$inferred_brief && /^\s*\/\/\s*(\S.+?)\s*$/ ) {
            $inferred_brief = $1;
        }
        if ( $_ =~ $language->{start} ) {
...
if ( $infer_module && scalar(@modules) == 0 ) {
    my $name = $source_link || 'source';
    $name =~ s{.*/}{};
    $name =~ s{\.[^.]+$}{};
    my $f = {
        name   => $name,
        source => $source_link,
        data   => $inferred_brief || 'No description supplied.',
        functions => [],
        methods => [],
        id => make_id($name, 'module'),
    };
